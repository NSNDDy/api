...
public class CalendarService {
    ...
    public ResponseEntity<ApiResponse<List<Map<String, Object>>>> getList(String dateIso,
                                                                          String fromIso,
                                                                          String toIso) {
        User me = currentUser();
        ...
        if (fromIso != null && toIso != null && !fromIso.isEmpty() && !toIso.isEmpty()) {
            ...
            todos = calendarTodoRepository.findByUser_IdAndDateBetweenOrderByDateAscCreatedAtAsc(me.getId(),
                                                                                                 from,
                                                                                                 to);
        } else if (dateIso != null && !dateIso.isEmpty()) {
            LocalDate date = LocalDate.parse(dateIso);
            todos = calendarTodoRepository.findByUser_IdAndDateOrderByCreatedAtAsc(me.getId(),
                                                                                   date);
        } else {
    ...
    public ResponseEntity<ApiResponse<List<Map<String, Object>>>> getAll() {
        User me = currentUser();
        List<Map<String, Object>> items = calendarTodoRepository.findByUser_IdOrderByDateAscCreatedAtAsc(me.getId()).stream().map(this::toDto).collect(Collectors.toList());
        return ResponseEntity.ok(ApiResponse.success("Todos",
                                                     items));
    }


    public ResponseEntity<ApiResponse<Map<String, Object>>> create(Map<String, Object> body) {
        User me = currentUser();
        ...
        CalendarTodo entity = CalendarTodo.builder().user(me).date(date).text(text).project(project).priority(priority).status(status).done(done).build();
        CalendarTodo saved = calendarTodoRepository.save(entity);
        return ResponseEntity.ok(ApiResponse.success("Created",
                                                     toDto(saved)));
    }

    public ResponseEntity<ApiResponse<Map<String, Object>>> update(Long id,
